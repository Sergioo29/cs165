...
void Game :: draw(const Interface & ui)
{
   ...
   if (lander.isLanded())
   {
      drawText(Point(), "WIN!! You've successfully landed!");
   }

   if (!lander.isAlive())
   {
      drawText(Point(), "FAIL! You have crashed!");
   }
   ...
   fuelLocationName.setX(topLeft.getX() + 5);
   fuelLocationName.setY(topLeft.getY() - 15);   
   drawText(fuelLocationName, "Fuel:");
   
   Point instructions;
   instructions.setX(topLeft.getX() + 238);
   instructions.setY(topLeft.getY() - 15);   
   drawText(instructions, "Land slowly on the platform");
   
   Point instructions2;
   instructions2.setX(topLeft.getX() + 238);
   instructions2.setY(topLeft.getY() - 30);   
   drawText(instructions2, "Use ARROW KEYS to thrust");
